...
void loop() {
  ...
  if (currtime - lastUpdate >= updateInterval) {
    ...
    currentTemp = (To + 273.0) * beta / (beta + (To + 273.0) * log(Rth / Ro)) - 273.0 - 24.46;

    error_temp = setTemp - currentTemp;
    KIinterror_temp += KI_temp * error_temp * deltat;
    KIinterror_temp = constrain(KIinterror_temp, 0, 3000);
    
    float heaterPower = round((Kp_temp * error_temp + KIinterror_temp) * kHeater);
    heaterPWM = constrain((int)heaterPower, 0, 3000 * 0.8);
    analogWrite(HEATER_PWM_PIN, 1023);
